...
because `requires` and `receives` are meant to be *ARGUMENT* validators. This also means several
things:
...
Around hooks in a command are sadly no different, as they just try to be easy to understand.

First things first, when you use an around hook you must compromise to *ALWAYS* have a `yield` at a
point of your code to call the next step. If you don't your command will never be called.

Now, when there are multiple around hooks the first one will call the second one and so forth until
the command is called. This means that before the `yield` the code is run in the order the arounds
were, but after the `yield` it is run in the *REVERSE* order.
...
----
around do
  puts "First before yield"
  yield
  puts "First after yield"
end

around do
  puts "Second before yield"
  yield
  puts "Second after yield"
end
